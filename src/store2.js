...
function localStore(config, emitter, store) {
  ...
  function clear(key) {
    ...
    const record = store[key];
    if (record !== undefined) {
      if (record.head) record.head.tail = record.tail;
      if (record === head) head = record.tail;
      if (record.hits === topHitter) {
        if (record.tail) topHitter = record.tail.hits;
        else if (record.head) topHitter = record.head.hits;
        else topHitter = 0;
      }
      if (record.tail) record.tail.head = record.head;
